...
func TestGetUrl(t *testing.T) {
	m := NewProvider("test", "API_KEY")
	assert.Equal(t, "https://api.openai.com/v1/chat/completions", m.GetUrl())
}
...
func TestPreparePayload(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.JSONEq(t, `{"model":"gpt-4o","messages":[{"role":"system","content":"System Prompt"},{"role":"user","content":"Question"},{"role":"assistant","content":"Answer"}]}`, payload)
}
